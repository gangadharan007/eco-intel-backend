...
# Optional: If you proxy waste-image AI to Railway, set this (no TensorFlow on Vercel)
...
def db_health():
    ...
    mysql.connector expects host/user/password/database style args. [web:568]
    """
    conn = get_db_connection()
    if not conn:
...
def waste_route():
    ...
    """
    # Mode 1: proxy to Railway AI (recommended; TensorFlow too big for Vercel)
    if WASTE_AI_URL:
        ...
        try:
            ...
            # Optional: store prediction as waste_data entry (example)
            # If you want this, ensure your waste_data schema matches.
            # predicted = payload.get("predicted_waste_type")
            # if predicted:
            #     save_waste_data(predicted, 1)
    ...
    # Mode 2: manual JSON insert into waste_data
    try:
        data = request.get_json() or {}
...
def crop_recommend():
    try:
        ...
        elif temperature and temperature > 25 and season.lower() == "rabi":
